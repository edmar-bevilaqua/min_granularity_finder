...
SUPPORTED_EXTENSIONS = ['.csv', '.txt', '.xls', '.xlsx', '.xlsm', '.xlsb', '.odf', '.ods', '.odt']
...
def evaluate_granularities(df, selected_columns, top_k=2):
    ...
    if not available_columns:
        st.warning("No candidate columns for granularity (all are dates, metrics or aggregated).")
        return []
    n_rows = len(df)
    granularities = []
...
""")
uploaded_file = st.file_uploader("import", type=SUPPORTED_EXTENSIONS, label_visibility="hidden")
if uploaded_file is not None:
    analyzer = GranularityAnalyzer()
    df = analyzer.load_file(uploaded_file)
    if df is not None:
        ...
        mode = st.radio("", ["Automatic (all columns)", "Manual (choose columns)"])

        if mode == "Automatic (all columns)":
            forbidden = list(set(detect_metric_columns(df) + detect_date_columns(df) + detect_aggregated_columns(df)))
            if forbidden:
                st.info(f"The columns below seem to be dates, metrics or already aggregated keys and will not be used as key:\n{forbidden}")
            ...
            selected_columns = st.multiselect("Choose columns for granularity analysis:", columns)
            if selected_columns:
                selected_metrics = [c for c in selected_columns if c in detect_metric_columns(df)]
                if selected_metrics:
                    st.info(f"The columns {selected_metrics} seem to be metrics or dependent variables and will not be used as key.")
